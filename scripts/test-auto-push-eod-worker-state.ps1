<#
.SYNOPSIS
    Smoke-Test fuer den Jarvis-Agent-Backstop in auto-push-eod.ps1 (Phase 11).
...
.DESCRIPTION
    ...
    und prueft per Log-Output, dass der Jarvis-Agent-Branch korrekt geskippt
    wird (bzw. mit -JarvisAgentWarnOnly nur warnen).

    Idempotent: temp-Repo wird vor + nach dem Test geloescht.

.EXAMPLE
    powershell -NoProfile -ExecutionPolicy Bypass -File scripts/test-auto-push-eod-worker-state.ps1
...
try {
    ...
    git branch feature/jarvis-agent-active 2>&1 | Out-Null

    # Worktrees auschecken
    git worktree add $wtClean feature/clean 2>&1 | Out-Null
    git worktree add $wtOC feature/jarvis-agent-active 2>&1 | Out-Null

    # In wtOC eine "aktive" Jarvis-Agent-Session simulieren
    $stateDir = Join-Path $wtOC ".openclaw_state\sess-deadbeef"
    New-Item -ItemType Directory -Path $stateDir -Force | Out-Null
    "fake jarvis-agent run" | Out-File -FilePath (Join-Path $stateDir "run.log") -Encoding utf8
    ...
    $output1 = & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -RepoRoot $repoRoot -DryRun -JarvisAgentActiveMinutes 60 2>&1 | Out-String

    Assert-Contains $output1 "Jarvis-Agent-Backstop aktiv" "Backstop wird aktiviert"
    Assert-Contains $output1 "Jarvis-Agent-Backstop trifft Branch 'feature/jarvis-agent-active'" "Aktiver Jarvis-Agent-Worktree wird erkannt"
    ...
    $output2 = & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -RepoRoot $repoRoot -DryRun -JarvisAgentActiveMinutes 60 -JarvisAgentWarnOnly 2>&1 | Out-String
    ...
    # --- Test 3: Disabled (JarvisAgentActiveMinutes=0) ---
    Write-Host "`n=== TEST 3: Disabled-Modus ===" -ForegroundColor Cyan
    $output3 = & powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath -RepoRoot $repoRoot -DryRun -JarvisAgentActiveMinutes 0 2>&1 | Out-String

    Assert-NotContains $output3 "Jarvis-Agent-Backstop aktiv" "Backstop ist deaktiviert"
    Assert-NotContains $output3 "Jarvis-Agent-Backstop trifft" "Keine Branches werden geblockt"
