#!/usr/bin/env python3
"""Generate the promo-video voiceover and a deterministic timeline.

Pipeline:
  1. Read the authored script (vo-script-promo.json): a voice, a rate, timing knobs,
     and a list of scenes, each with ordered lines (narration | command).
  2. Synthesize one MP3 per line with edge-tts (free Microsoft neural voices,
     no API key) into video/public/vo/<id>.mp3.
  3. Measure each clip's real duration with ffprobe.
  4. Lay out a frame-accurate timeline (scene durations + per-line local start
     frames + absolute audio placement) and write it to
     video/src/intro/generated/timeline.json, which the Remotion composition
     imports so picture and voice stay in sync.

Run from the `video/` directory:  python scripts/gen_vo.py
Re-run any time the script text changes; it is idempotent.
"""
...
SCRIPT_PATH = ROOT / "vo-script-promo.json"
VO_DIR = ROOT / "public" / "vo-promo"
OUT_PATH = ROOT / "src" / "intro" / "promo" / "generated" / "timeline.json"
