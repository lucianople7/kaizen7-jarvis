...
class Event:
    ...
    # v2: routes events to the right network client over SSE. "default" keeps
    # the in-process v1 behaviour working unchanged.
...
def event_to_wire(ev: Event) -> dict[str, Any]:
    """Serialise an event to a JSON-safe dict for SSE transport (v2).

    ``type`` is the class name; UUIDs become strings and enums become their
    value. The SSE hub uses this to push events to network clients.
    """
