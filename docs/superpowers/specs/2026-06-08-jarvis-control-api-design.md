...
3. **Force-spawn interception.** "wechsel auf / switch to" is an action verb → `_should_force_spawn` may route the utterance to a contextless Jarvis-Agent worker instead of the tool → silent no-op.
